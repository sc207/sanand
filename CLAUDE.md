...
- **styles.css puts the figure disc at `align-self: flex-end` below 600px**, which is right
  for the column card it lays out there and wrong for the row this app overrides it to —
  flex-end then means the BOTTOM, so the disc floated low-left with the label above it
  beside nothing. Half an override is worse than neither; it is pinned to the first line at
  every small width now. **Keep the disc.** The trust asked for it, it is how a figure is
  recognised at a glance, and hiding it on phones was the wrong call — it is 24px there
  rather than 28 because at the larger size it took a fifth of a 171px cell.
- **The figure strip sets `grid-auto-rows: 1fr` below 900px.** A sub-label like "10 devotees
  registered" wraps in a 171px cell where "5 sevarthi pending" does not, so the rows came
  out at 79 / 97 / 80 and the strip read as ragged even once every tile in it was right.
  Equal rows cost a few pixels and make it a grid again — prefer that to shrinking the type
  or shortening the trust's own wording to make things fit.
